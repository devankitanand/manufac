...
const Yeardata: React.FC = () => {
  ...
  useEffect(() => {
    fetch('/cropData.json') // Adjust the path to match your folder structure
  ...
  return (
    <Table striped highlightOnHover style={{ borderCollapse: 'collapse', width: '100%' }}>
      <thead>
        <tr>
          <th style={{ border: `1px solid ${theme.colors.gray[5]}`, padding: theme.spacing.xs, background: theme.colors.gray[0] }}>Year</th>
          <th style={{ border: `1px solid ${theme.colors.gray[5]}`, padding: theme.spacing.xs, background: theme.colors.gray[0] }}>Crop with Maximum Production</th>
          <th style={{ border: `1px solid ${theme.colors.gray[5]}`, padding: theme.spacing.xs, background: theme.colors.gray[0] }}>Crop with Minimum Production</th>
        </tr>
      </thead>
      <tbody>
        {data.map((row) => (
          <tr key={row.Year}>
            <td style={{ border: `1px solid ${theme.colors.gray[5]}`, padding: theme.spacing.xs ,textAlign: 'center', verticalAlign: 'middle'  }}>{row.Year}</td>
            <td style={{ border: `1px solid ${theme.colors.gray[5]}`, padding: theme.spacing.xs ,textAlign: 'center', verticalAlign: 'middle' }}>{row.maxProductionCrop}</td>
            <td style={{ border: `1px solid ${theme.colors.gray[5]}`, padding: theme.spacing.xs ,textAlign: 'center', verticalAlign: 'middle' }}>{row.minProductionCrop}</td>
          </tr>
        ))}
      </tbody>
    </Table>
